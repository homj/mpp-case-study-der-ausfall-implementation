...
- Done: domain engine with tests (`packages/domain`, 62 tests), DB schema + seed + idempotent export ingest with data-issue detection (`packages/db`), explicit API with OpenAPI (`apps/api`: absences, the reschedule-task queue, quick actions, ingest + reconciliation, data issues), fake Termino client + fake notifier behind an outbox, front-desk web app wired to the live API (`apps/web`), `docker compose up` for db + api + web.
- Stopped at: the second half of the front-desk UI. The queue, the case sheet, the new-absence dialog, the absences page, and the Termino ingest button run against the real API. Not built: the undo dialog for automated actions, the data-issues panel, and the outbox panel — the API serves all three, the UI does not show them yet.

1. ~~Wire `apps/web` to the API following the two-flow prototype (queue across absences, absence modal, side drawer); add `web` to docker compose.~~ Done: `GET /tasks` serves one ranked queue across all absences; `/queue`, `/absences`, the "Neuer Ausfall" dialog, and the quick actions run live; `web` is in docker compose. Remaining in the UI: undo of an automated action, the data-issues panel, the outbox panel, and the "all appointments of this patient" section inside the case sheet.
